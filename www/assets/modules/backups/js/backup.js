...
$( document ).ready(function() {
    ...
    $(document).on('click', '.BackupRestore', function () {
        var id = $(this).find('[name=id]').val();
        var $this = $(this);
        $this.button('loading');
        var files = $this.data('files');
        var dataForm = {files:files};
        if(confirm('Все текущие файлы сайта будут перезаписаны файлами из коппии. Все равно продолжить?')){
            $.ajax({
                type: "POST",
                url: '/backups/backup/restorefiles',
                data: dataForm,
                success: function (res) {
                    var res = JSON.parse(res);
                    if (res.error == "0") {
    ...
    $(document).on('click', '.BackupDbRestore', function () {
        var id = $(this).find('[name=id]').val();
        var $this = $(this);
        $this.button('loading');
        var dbfile = $this.data('dbfile');
        var dataForm = {dbfile:dbfile};
        if(confirm('Текущая база будет очищена и заменена этой копией. Все равно продолжить?')){
            $.ajax({
                type: "POST",
                url: '/backups/backup/restoredb',
                data: dataForm,
                success: function (res) {
                    var res = JSON.parse(res);
                    if (res.error == "0") {
