...
<div class="panel panel-body">
    ...
    <form action="/block/save/<?= empty($data['item']->id)?"0":$data['item']->id; ?><?= isset($data['pid']) ? "/" . $data['pid'] : "" ?>"
          method="post" id="editBlockForm">
        <div class="form-group">
            ...
            <input type="text" data-name="name" required class="form-control" name="name" id="block-name"
                   value="<?= htmlspecialchars($data['item']->name) ?>">
        </div>
        <div class="form-group">
            ...
            <input class="form-control" data-name="title" name="title" type="text" id="block-title"
                   value="<?= htmlspecialchars($data['item']->title) ?>">
        </div>
        <div class="form-group">
            <label for="block-content" class="control-label">Содержание</label>
            <textarea class="form-control" name="content" id="block-content"
                      rows="10"><?= htmlspecialchars($data['item']->content) ?></textarea>
        </div>
        <div class="checkbox">
            <label>
                <input
                        style="position: absolute; margin-left: -20px;"
                        type="checkbox" <?= $data['item']->is_editor_enabled ? 'checked' : '' ?>
                        name="is_editor_enabled" id="toggleEditor"> Визуальный редактор
            </label>
        </div>
