...
<div class="page-header clearfix panel">
    <div class="col-sm-6 pull-left" style="font-size: 17px; padding-top: 4px;">
        <div class="pull-left">
            <?php
            if (!empty($data['Question']->title)) {
                ?>Вопрос: <?= $data['Question']->title ?><?php
            }
            ?>
        </div>
    </div>
    <div class="col-sm-6 pull-right" style="text-align: right;">
        <a href="/faq/answers/form/0/ajax/<?= (!empty($data['Question']->id)) ? $data['Question']->id : "" ?>"
           data-title="Добавление" class="btn btn-xs btn-success ajax">Добавить</a>
    </div>
</div>
<div class="panel-body panel">
    <table class="table table-striped table-hover table-bordered responsive TableProject"
           data-id="<?= (!empty($data['Question']->id)) ? $data['Question']->id : 0 ?>">
        <thead>
        <tr>
            <th width="50">#</th>
            <th>Пользователь</th>
