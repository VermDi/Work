...
class Whois
{
    ...
    {
        if (empty($domain)) {
            $this->error[] =  ["++[ERROR_NOTHING_ENTERED]++"];
            return false;
        }
        ...
        if (!array_key_exists($tld, $this->whoisServers)) {
            $this->error[] = ["++[ERROR_UNSUPPORTED_TLD]++"];
            return false;
        }
        ...
        while (!feof($f)) {
            $data[] = $this->rawdata[] = rtrim(fgets($f), "\n");        }
                ...
                foreach ($stringsExistToMeanWaitSomeTime as $stringExistToMeanWaitSomeTime => $sscanfFullString) {
                    if (strpos($eachLine, $stringExistToMeanWaitSomeTime) !== false) {
                        if ($sscanfFullString == "N/A") {
                            ...
                            if (!empty($second)) {
                                $this->error[] =  ["++[PLZ_WAIT_" . $second . "_SEC]++"];
                                return false;
                            } else {
                                $this->error[] =  ["++[PLZ_WAIT_SOME_TIME]++"];
                                return false;
                            }
                        }
                    }
    ...
    private function parse_domain($data, $domain, $tld)
    {
        ...
        if ($this->returnType != "text") {
            if (empty($parseResult)) {
                $this->error[] =  ["++[ERROR_NOTHING_IN_ARRAY]++"];
                return false;
            }
        }
