...
namespace src\dbtree;

use core\Model;

/**
 * @property-read Model $db
 * DbTree.php
...
class DbTree
{
    ...
    public function GetNode($nodeId, $fields = '*')
    {
        ...
        $result = $this->db->from($this->table)->where([$this->tableId => (int)$nodeId])->getOne();
//        if (false === $result) {
...
//        }
    ...
    public function GetChildren($nodeId, $fields = '*', $condition = array())
    {
        $condition = $this->PrepareCondition($condition, false, 'A.');
        $fields    = $this->PrepareSelectFields($fields, 'A');
        ...
        $sql    = 'SELECT ' . $fields . ' FROM ' . $this->table . ' AS A';
        $sql    .= ' WHERE ' . $this->tableLeft . ' > ' . $left_id . ' AND ' . $this->tableRight . ' < ' . $right_id;
        $sql    .= $condition . ' ORDER BY ' . $this->tableLeft;
        $result = $this->db->query($sql);
...
//        }
    ...
    {
        return $this->db->max('right_key', 'max_right_key')->where($condition)->getOne();
    }
    ...
    {
        if ($node_info = $this->GetNode($parentId)) {
            $data = $this->createNode($node_info, $data, $condition);
        } else {
    ...
    private function createNode($parent_node, $data, $condition = array())
    {
        $tbl_right = $this->tableRight;
        $tbl_level = $this->tableLevel;
        $right_id  = $parent_node->$tbl_right;
        $level     = $parent_node->$tbl_level;
    ...
    public function DeleteAll($nodeId, $condition = '')
    {
        ...
        $tblleft  = $this->tableLeft;
        $tblright = $this->tableRight;
        $left_id  = $node_info->$tblleft;
        $right_id = $node_info->$tblright;
