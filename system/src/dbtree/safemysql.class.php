 ...
 * ?l like %...   - complex placeholder for like (left %)
 * ?r like ...%   - complex placeholder for like (right %)
 * ?d like %...%  - complex placeholder for like (left and right)
 * and
...
class SafeMySQL
{
    ...
    private function prepareQuery($args)
    {
        ...
        $array = preg_split('~(\?[nsiuaplrd])~u', $raw, null, PREG_SPLIT_DELIM_CAPTURE);
        $anum = count($args);
        ...
        foreach ($array as $i => $part) {
            ...
            switch ($part) {
                ...
                case '?s':
                    ...
                    break;
                case '?l':
                    $part = $this->escapeLikeString($value, 'l');
                    break;
                case '?r':
                    $part = $this->escapeLikeString($value, 'r');
                    break;
                case '?d':
                    $part = $this->escapeLikeString($value, 'd');
                    break;
                case '?i':
    ...
    private function escapeLikeString($value, $pos = 'r')
    {
        if ($value === NULL) {
            return 'NULL';
        }
